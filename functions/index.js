...
function analysisOfAnswers2(dataMatrix, questions, metaData) {
    ...
    questionsSet.forEach((question, index) => {
        ...
        if (question.respFreqString) {
            questionsSet[index].respFreq = frequencyCount(questionsSet[index].respFreqString)
        }
    })
    return questionsSet;
}
    ...
    for (let i = 0; i < responses.length; i++) {
        ...
        for (let j = 0; j < submission.length; j++) {
            ...
            if (question.choices) {
                ...
                questionsArray[j].respFreqString = questionsArray[j].respFreqString + "" + choice.submittedAnswer
                if (questionsArray[j].respFreqString) {
                    questionsArray[j].respFreq = frequencyCount(questionsArray[j].respFreqString)
                }
